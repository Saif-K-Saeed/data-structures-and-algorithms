...
const addPurchases = (arr) => {
  // Solution code here...
  return arr.reduce((add, inx) => {
    return add = add + inx.purchasePrice;
  }, 0);
};
...
const returnNames = (arr) => {
  // Solution code here...
  const newArry = arr.reduce((star, inx) => {
    star.push(inx.name);
    return star;
  }, []);
  return newArry;
