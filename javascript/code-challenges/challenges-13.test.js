...
const longestString = (arr) => {
  ...
  let elemnt = '';
  arr.map(str => {
    if (str.length > elemnt.length)
      elemnt = str;
  });

  return arr.indexOf(elemnt);
};
...
const firstLetters = (arr) => {
  // Solution code here...
  let firstLatter = arr.map(letter => letter[0]);
  return firstLatter;
};
...
const findHappiness = (arr) => {
  ...
  let find = /^:\)/g;
  let find2 = /:\)/g;
  let match = arr.filter(elemnt => {
    if ((find.test(elemnt)) || (find2.test(elemnt)))
      return elemnt;
  });
  return match;
};
